...
function EditSpecTestSet(props) {
  const history = useHistory();
  const [editSpecialTestSet, { error }] = useMutation(EDIT_SPECIAL_TEST_SET);
  const [specTestSet, setSpecTestSet] = useState([]);

  useEffect(() => {
    setSpecTestSet(JSON.parse(props?.location?.state?.data?.questions));
  }, []);
              ...
              <div style={{ width: "100%", marginBottom: 30 }}>
                ...
                >
                  ...
                  <Select
                    ...
                    defaultValue={() => {
                      let answer = 0;
                      item?.answerOptions?.forEach((item, index) => {
                        if (item?.isCorrect) {
                          answer = index;
                        }
                      });
...
export default EditSpecTestSet;
